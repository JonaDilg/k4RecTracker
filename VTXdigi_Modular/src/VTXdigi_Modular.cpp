...
void VTXdigi_Modular::InitServicesAndGeometry() {
  ...
  debug() << " - Successfully retrieved all necessary services and detector elements." << endmsg;
}
...
void VTXdigi_Modular::InitLayersAndSensors() {
  ...
  *  - */
  ...
  if (!m_layersToDigitize.value().empty()) {
    /* ie. the relevant layers are specified in the Gaudi property */
    ...
    debug() << " - Digitizing " << m_layerCount << " layers as specified in Gaudi property by the user." << endmsg;
  }
  else {
    /* digitize all layers in m_subDetector */
    m_layerCount = availableLayers.size();
    debug() << " - Digitizing all " << m_layerCount << " layers found as children of subdetector " << m_subDetName.value() << "." << endmsg;
  }

  /* Get pixel pitch from the segmentation (from the readout that matches our simHitCollection) 
  *  I don't know of a better way to do this (that works for the IDEA detector model) */
  verbose() << " - Retrieving sensor pixel count from segmentation for subDetector \"" << m_subDetName.value() << "\"..." << endmsg;
  ...
  /* TODO: Check that local sensor coordinates (u,v,w) are correctly defined wrt. to the global coordinates (already done in VTXdigi_Modular in master branch) */

  /* Loop over all sensors in the subDetector and check that their geometry is consistent with the one we just retrieved
  ...
  for (const auto& [layerKey, layer] : m_subDetector.children()) {
    ...
    if (!m_layersToDigitize.value().empty()) {
      if (std::find(m_layersToDigitize.value().begin(), m_layersToDigitize.value().end(), layerNumber) == m_layersToDigitize.value().end()) {
        debug() << "   - Skipping layer " << layerKey << " (layerNumber " << layerNumber << ", volumeID " << layerVolumeID << " with " << layer.children().size() << " modules) as it is not in the LayersToDigitize list." << endmsg;
        continue;
      }
    }
    else {
        debug() << "   - Found layer \"" << layerKey << "\" (layerNumber " << layerNumber << ", volumeID " << layerVolumeID << ", " << layer.children().size() << " modules) for subDetector \"" << m_subDetName.value() << "\"." << endmsg;
    }
    ...
    for (const auto& [moduleKey, module] : layer.children()) {
      ...
      for (const auto& [sensorKey, sensor] : module.children()) {
        ...
        // for (const auto& [pixelKey, pixel]: sensor.children()) {
        // verbose() << "     - Found pixel \"" << pixelKey << "\" in sensor \"" << sensorKey << "\"." << endmsg;
        // }
        ...
        if (!membersDefined) {
          ...
          if (std::abs(sensorLength_u - m_sensorLength.at(0)) > 0.001 ||
            ...
            verbose() << "     - Found sensor: " << sensorKey << ", volumeID: " << sensorVolumeID << " (sensor " << sensorNumber << " in layer " << layerNumber << "). Dimensions are consistent." << endmsg;
          }
        } // if membersDefined
      } // loop over sensors
    } // loop over modules
  } // loop over layers
  debug() << " - Retrieved sensor parameters: area (" << m_sensorLength.at(0) << " x " << m_sensorLength.at(1) << ") mm, thickness " << m_sensorThickness << " mm, pixel pitch (" << m_pixelPitch.at(0) << " x " << m_pixelPitch.at(1) << ") mm, pixel count (" << m_pixelCount.at(0) << " x " << m_pixelCount.at(1) << "). All " << sensorNumber << " sensors in the relevant layers have these parameters." << endmsg;
} // InitLayersAndSensors()