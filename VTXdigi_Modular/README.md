...
- `ApplyEtaCorrection` - Apply an η-correction. Requires η-distributions for u and v (either input via points, or extracted from LUT). See [η correction](#η-correction). Defaults to false.
- `ExtractEtaDistributionFromChargeCollector` - Extract the eta correction from the charge collector automatically. Fails in initialization if the ised charge collector does not implement this functionality. Defaults to False.
- `EtaDistributionFile` - TODO, currently unused. File containing the numerical eta distribution points.
